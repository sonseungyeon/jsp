...
<body>
	...
	<%
		//System.out.println(name1); 자바 변수를 만드는 것은 아님.
	%>
	<%-- 
	...
	name1 : ${name1}
	<hr>
	<c:set var="names" value='<%=new String[]{"가렌","카타리나","샤코"} %>' scope="request"/>
	<%-- <c:set var="names" scope="request"><%=new String[]{"가렌","카타리나","샤코"} %></c:set>--%>
	names : ${names[0] },${names[1] },${names[2] }
	<hr>
	<c:set var="myList" value='<%=new ArrayList<String>() %>' scope="request"/>
	<hr>
	<%-- 이미 만들어져 있는 객체를 참조하는 것도 가능하다 --%>
	<%
		Map<String, String> skill = new HashMap<String, String>();
	%>
	<!-- property가 키값. 키값을 통해서 value를 가지고 온다. -->
	<c:set target="<%=skill %>" property="q" value="신비한화살"/>
	<%=skill.get("q")%>
	
	<c:set var="hobbyMap" value='<%=new HashMap<String, String>() %>' scope="request"></c:set>
	<% //request.getAttribute("hobbyMap"); %>
	<!-- 속성에 접근하기 쉬우므로 사용한다. -->
